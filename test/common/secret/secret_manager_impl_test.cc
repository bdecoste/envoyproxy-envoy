...
tls_certificate:
  ...
  Envoy::Tls::TlsCertificateConfigImpl tls_config(
...
TEST_F(SecretManagerImplTest, CertificateValidationContextSecretLoadSuccess) {
  ...
  Envoy::Tls::CertificateValidationContextConfigImpl cvc_config(
...
tls_certificate:
  ...
  Envoy::Tls::TlsCertificateConfigImpl tls_config(*secret_provider->secret());
  const std::string cert_pem =
      "{{ test_rundir }}/test/extensions/transport_sockets/tls/test_data/selfsigned_cert.pem";
  EXPECT_EQ(TestEnvironment::readFileToStringForTest(TestEnvironment::substitute(cert_pem)),
            tls_config.certificateChain());
