#include "extensions/transport_sockets/openssl/config.h"

#include "envoy/config/transport_socket/openssl/v2alpha/openssl.pb.h"
#include "envoy/config/transport_socket/openssl/v2alpha/openssl.pb.validate.h"
...
//#include "extensions/transport_sockets/openssl/grpc_openssl.h"
...
/*HandshakeValidator
createHandshakeValidator(const envoy::config::transport_socket::openssl::v2alpha::Openssl& config) {
...
createTransportSocketFactoryHelper(const Protobuf::Message& message, bool is_upstream) {
  auto config =
      MessageUtil::downcastAndValidate<const envoy::config::transport_socket::openssl::v2alpha::Openssl&>(
          message);

  if (is_upstream) {
    std::cout << "is_upstream " << is_upstream << " \n";
  }
  ...
  HandshakerFactory factory =
      [handshaker_service,
       is_upstream](Event::Dispatcher& dispatcher,
                    ...
                    const Network::Address::InstanceConstSharedPtr&) -> OpensslHandshakerPtr {
		...
		// in this use case. The validation will be performed by OpensslSocket with the validator.
    ...
    return std::make_unique<OpensslHandshaker>(std::move(handshaker_ptr), dispatcher);
  };*/

  return std::make_unique<OpensslSocketFactory>();
}

} // namespace

ProtobufTypes::MessagePtr OpensslTransportSocketConfigFactory::createEmptyConfigProto() {
  return std::make_unique<envoy::config::transport_socket::openssl::v2alpha::Openssl>();
}

Network::TransportSocketFactoryPtr
UpstreamOpensslTransportSocketConfigFactory::createTransportSocketFactory(
...
Network::TransportSocketFactoryPtr
DownstreamOpensslTransportSocketConfigFactory::createTransportSocketFactory(
    const Protobuf::Message& message, Server::Configuration::TransportSocketFactoryContext&,
    const std::vector<std::string>&) {
  return createTransportSocketFactoryHelper(message, /* is_upstream */ false);
}

static Registry::RegisterFactory<UpstreamOpensslTransportSocketConfigFactory,
                                 Server::Configuration::UpstreamTransportSocketConfigFactory>
    upstream_registered_;

static Registry::RegisterFactory<DownstreamOpensslTransportSocketConfigFactory,
