...
createClientSslTransportSocketFactory(const ClientSslTransportOptions& options,
  ...
  auto cfg = std::make_unique<Envoy::Extensions::TransportSockets::Tls::ClientContextConfigImpl>(
      tls_context, options.sigalgs_, mock_factory_ctx);
  static auto* client_stats_store = new Stats::TestIsolatedStoreImpl();
  return Network::TransportSocketFactoryPtr{
      new Envoy::Extensions::TransportSockets::Tls::ClientSslSocketFactory(
          std::move(cfg), context_manager, *client_stats_store)};
}
...
Network::TransportSocketFactoryPtr createUpstreamSslContext(ContextManager& context_manager) {
  ...
  NiceMock<Server::Configuration::MockTransportSocketFactoryContext> mock_factory_ctx;
  auto cfg = std::make_unique<Envoy::Extensions::TransportSockets::Tls::ServerContextConfigImpl>(
      tls_context, mock_factory_ctx);

  static Stats::Scope* upstream_stats_store = new Stats::TestIsolatedStoreImpl();
  return std::make_unique<Envoy::Extensions::TransportSockets::Tls::ServerSslSocketFactory>(
