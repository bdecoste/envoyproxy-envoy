...
#include "test/extensions/transport_sockets/ssl/ssl_test_utility.h"
#include "test/extensions/transport_sockets/ssl/test_data/no_san_cert_info.h"
#include "test/extensions/transport_sockets/ssl/test_data/san_dns3_cert_info.h"
...
TEST_F(SslContextImplTest, TestExpiredCert) {
  ...
  common_tls_context:
    tls_certificates:
      certificate_chain:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/expired_cert.pem"
      private_key:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/expired_key.pem"
)EOF";
...
TEST_F(SslContextImplTest, TestGetCertInformation) {
 ...
 "path": "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/no_san_cert.pem",
 "serial_number": ")EOF",
...
TEST_F(SslContextImplTest, TestGetCertInformationWithSAN) {
  ...
  common_tls_context:
    ...
    validation_context:
      trusted_ca:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/san_dns3_cert.pem"
)EOF";
  ...
  ClientContextSharedPtr context(manager_.createSslClientContext(store_, cfg));
  std::string ca_cert_json = absl::StrCat(R"EOF({
 "path": "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/san_dns3_cert.pem",
 "serial_number": ")EOF",
 ...
 "subject_alt_names": [
  ...
  std::string cert_chain_json = R"EOF({
 "path": "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/san_dns3_chain.pem",
 }
)EOF";
...
TEST_F(SslContextImplTest, AtMostOneRsaCert) {
  ...
  common_tls_context:
    ...
    - certificate_chain:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned2_cert.pem"
      private_key:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_key.pem"
  )EOF";
...
TEST_F(SslContextImplTest, AtMostOneEcdsaCert) {
  ...
  common_tls_context:
    ...
    - certificate_chain:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned2_ecdsa_p256_cert.pem"
      private_key:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_ecdsa_p256_key.pem"
  )EOF";
  MessageUtil::loadFromYaml(TestEnvironment::substitute(tls_context_yaml), tls_context);
...
TEST_F(SslServerContextImplTicketTest, TicketKeyInvalidLen) {
  ...
  session_ticket_keys:
    keys:
      filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/ticket_key_a"
      filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/ticket_key_wrong_len"
)EOF";
...
TEST_F(SslServerContextImplTicketTest, TicketKeyInvalidCannotRead) {
  ...
  session_ticket_keys:
    keys:
      filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/this_file_does_not_exist"
)EOF";
  ...
  common_tls_context:
    ...
    validation_context:
      trusted_ca:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/ca_cert.pem"
      crl:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/not_a_crl.crl"
)EOF";
...
TEST(ClientContextConfigImplTest, RSA1024Cert) {
  ...
  const std::string tls_certificate_yaml = R"EOF(
  certificate_chain:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_rsa_1024_cert.pem"
  private_key:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_rsa_1024_key.pem"
  )EOF";
  MessageUtil::loadFromYaml(TestEnvironment::substitute(tls_certificate_yaml),
                            *tls_context.mutable_common_tls_context()->add_tls_certificates());
  ClientContextConfigImpl client_context_config(tls_context, factory_context);
...
TEST(ClientContextConfigImplTest, RSA3072Cert) {
  ...
  const std::string tls_certificate_yaml = R"EOF(
  certificate_chain:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_rsa_3072_cert.pem"
  private_key:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_rsa_3072_key.pem"
  )EOF";
  MessageUtil::loadFromYaml(TestEnvironment::substitute(tls_certificate_yaml),
                            *tls_context.mutable_common_tls_context()->add_tls_certificates());
  ClientContextConfigImpl client_context_config(tls_context, factory_context);
...
TEST(ClientContextConfigImplTest, RSA4096Cert) {
  ...
  const std::string tls_certificate_yaml = R"EOF(
  certificate_chain:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_rsa_4096_cert.pem"
  private_key:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_rsa_4096_key.pem"
  )EOF";
  MessageUtil::loadFromYaml(TestEnvironment::substitute(tls_certificate_yaml),
                            *tls_context.mutable_common_tls_context()->add_tls_certificates());
  ClientContextConfigImpl client_context_config(tls_context, factory_context);
...
TEST(ClientContextConfigImplTest, NonP256EcdsaCert) {
  ...
  const std::string tls_certificate_yaml = R"EOF(
  certificate_chain:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_ecdsa_p384_cert.pem"
  private_key:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_ecdsa_p384_key.pem"
  )EOF";
  MessageUtil::loadFromYaml(TestEnvironment::substitute(tls_certificate_yaml),
                            *tls_context.mutable_common_tls_context()->add_tls_certificates());
  ClientContextConfigImpl client_context_config(tls_context, factory_context);
...
TEST(ClientContextConfigImplTest, ValidationContextNotReady) {
  ...
  client_cert->mutable_certificate_chain()->set_filename(TestEnvironment::substitute(
      "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_cert.pem"));
  client_cert->mutable_private_key()->set_filename(TestEnvironment::substitute(
      "{{ test_rundir }}/test/extensions/transport_sockets/ssl/test_data/selfsigned_key.pem"));
  NiceMock<LocalInfo::MockLocalInfo> local_info;
  NiceMock<Event::MockDispatcher> dispatcher;
  NiceMock<Runtime::MockRandomGenerator> random;
  Stats::IsolatedStoreImpl stats;
