...
TEST_F(OpensslContextImplTest, TestdNSNameMatching) {
  ...
  EXPECT_TRUE(Envoy::Ssl::ContextImpl::dNSNameMatch("lyft.com", "lyft.com"));
  EXPECT_TRUE(Envoy::Ssl::ContextImpl::dNSNameMatch("a.lyft.com", "*.lyft.com"));
  EXPECT_TRUE(Envoy::Ssl::ContextImpl::dNSNameMatch("a.b.lyft.com", "*.lyft.com"));
  EXPECT_FALSE(Envoy::Ssl::ContextImpl::dNSNameMatch("foo.test.com", "*.lyft.com"));
  EXPECT_FALSE(Envoy::Ssl::ContextImpl::dNSNameMatch("lyft.com", "*.lyft.com"));
  EXPECT_FALSE(Envoy::Ssl::ContextImpl::dNSNameMatch("alyft.com", "*.lyft.com"));
  EXPECT_FALSE(Envoy::Ssl::ContextImpl::dNSNameMatch("alyft.com", "*lyft.com"));
  EXPECT_FALSE(Envoy::Ssl::ContextImpl::dNSNameMatch("lyft.com", "*lyft.com"));
...
TEST_F(OpensslContextImplTest, TestCipherSuites) {
  ...
  Envoy::Ssl::ClientContextConfigImpl cfg(tls_context, factory_context_);
...
TEST_F(OpensslContextImplTest, TestExpiredCert) {
  ...
  common_tls_context:
    tls_certificates:
      certificate_chain:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/expired_cert.pem"
      private_key:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/expired_key.pem"
)EOF";
...
TEST_F(OpensslContextImplTest, TestGetCertInformation) {
  ...
  common_tls_context:
    ...
    validation_context:
      trusted_ca:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ca_cert.pem"
)EOF";
 ...
 "path": "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ca_cert.pem",
...
TEST_F(OpensslContextImplTest, TestGetCertInformationWithSAN) {
  ...
  common_tls_context:
    tls_certificates:
      certificate_chain:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/san_dns_chain3.pem"
      private_key:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/san_dns_key3.pem"
    validation_context:
      trusted_ca:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/san_dns_cert3.pem"
)EOF";
  ...
  Envoy::Ssl::ClientContextSharedPtr context(manager_.createSslClientContext(store_, cfg));
  std::string ca_cert_json = R"EOF({
 "path": "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/san_dns_cert3.pem",
 ...
 "subject_alt_names": [
  ...
  std::string cert_chain_json = R"EOF({
 "path": "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/san_dns_chain3.pem",
 }
)EOF";
...
public:
  ...
  void loadConfigV2(envoy::api::v2::auth::DownstreamTlsContext& cfg) {
    ...
    Envoy::Ssl::ServerContextConfigImpl server_context_config(cfg, factory_context);
  ...
  void loadConfigYaml(const std::string& yaml) {
    ...
    MessageUtil::loadFromYaml(TestEnvironment::substitute(yaml), tls_context);
    NiceMock<Server::Configuration::MockTransportSocketFactoryContext> factory_context;
    Envoy::Ssl::ServerContextConfigImpl cfg(tls_context, factory_context);
    loadConfig(cfg);
  }
};
...
TEST_F(SslServerContextImplTicketTest, TicketKeySuccess) {
  ...
  session_ticket_keys:
    keys:
      filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ticket_key_a"
      filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ticket_key_b"
)EOF";
...
TEST_F(SslServerContextImplTicketTest, TicketKeyInvalidLen) {
  ...
  session_ticket_keys:
    keys:
      filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ticket_key_a"
      filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ticket_key_wrong_len"
)EOF";
...
TEST_F(SslServerContextImplTicketTest, TicketKeyInvalidCannotRead) {
  ...
  session_ticket_keys:
    keys:
      filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/this_file_does_not_exist"
)EOF";
...
TEST_F(SslServerContextImplTicketTest, CRLSuccess) {
  ...
  common_tls_context:
    tls_certificates:
      certificate_chain:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/san_dns_cert.pem"
      private_key:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/san_dns_key.pem"
    validation_context:
      trusted_ca:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ca_cert.pem"
      crl:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ca_cert.crl"
)EOF";
  EXPECT_NO_THROW(loadConfigYaml(yaml));
}
  ...
  common_tls_context:
    ...
    validation_context:
      trusted_ca:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ca_cert.pem"
      crl:
        filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/not_a_crl.crl"
)EOF";
...
TEST(ClientContextConfigImplTest, EmptyServerNameIndication) {
  ...
  EXPECT_THROW_WITH_MESSAGE(
	  Envoy::Ssl::ClientContextConfigImpl client_context_config(tls_context, factory_context), EnvoyException,
...
TEST(ClientContextConfigImplTest, InvalidCertificateHash) {
  ...
  Envoy::Ssl::ClientContextConfigImpl client_context_config(tls_context, factory_context);
...
TEST(ClientContextConfigImplTest, P256EcdsaCert) {
  ...
  certificate_chain:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_cert_ecdsa_p256.pem"
  private_key:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_key_ecdsa_p256.pem"
  )EOF";
...
TEST(ClientContextConfigImplTest, NonP256EcdsaCert) {
  ...
  const std::string tls_certificate_yaml = R"EOF(
  certificate_chain:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_cert_ecdsa_p384.pem"
  private_key:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_key_ecdsa_p384.pem"
  )EOF";
  MessageUtil::loadFromYaml(TestEnvironment::substitute(tls_certificate_yaml),
                            *tls_context.mutable_common_tls_context()->add_tls_certificates());
  Envoy::Ssl::ClientContextConfigImpl client_context_config(tls_context, factory_context);
...
TEST(ClientContextConfigImplTest, MultipleTlsCertificates) {
  ...
  const std::string tls_certificate_yaml = R"EOF(
  certificate_chain:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_cert.pem"
  private_key:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_key.pem"
  )EOF";
  MessageUtil::loadFromYaml(TestEnvironment::substitute(tls_certificate_yaml),
                            *tls_context.mutable_common_tls_context()->add_tls_certificates());
  MessageUtil::loadFromYaml(TestEnvironment::substitute(tls_certificate_yaml),
...
TEST(ClientContextConfigImplTest, ValidationContextNotReady) {
  ...
  client_cert->mutable_certificate_chain()->set_filename(TestEnvironment::substitute(
      "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_cert.pem"));
  client_cert->mutable_private_key()->set_filename(TestEnvironment::substitute(
      "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_key.pem"));
  NiceMock<LocalInfo::MockLocalInfo> local_info;
  NiceMock<Event::MockDispatcher> dispatcher;
  NiceMock<Runtime::MockRandomGenerator> random;
  Stats::IsolatedStoreImpl stats;
...
tls_certificate:
  ...
  const std::string cert_pem = "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_cert.pem";
  EXPECT_EQ(TestEnvironment::readFileToStringForTest(TestEnvironment::substitute(cert_pem)),
            client_context_config.tlsCertificates()[0].get().certificateChain());
  const std::string key_pem = "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_key.pem";
...
TEST(ClientContextConfigImplTest, PasswordProtectedTlsCertificates) {
  ...
  tls_certificate->mutable_certificate_chain()->set_filename(TestEnvironment::substitute(
      "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/password_protected_cert.pem"));
  tls_certificate->mutable_private_key()->set_filename(TestEnvironment::substitute(
      "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/password_protected_key.pem"));
  tls_certificate->mutable_password()->set_filename(
      TestEnvironment::substitute("{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/password.txt"));
  ...
  EXPECT_EQ(TestEnvironment::readFileToStringForTest(TestEnvironment::substitute(key_pem)),
            client_context_config.tlsCertificates()[0].get().privateKey());
  const std::string password_file = "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/password.txt";
...
TEST(ClientContextConfigImplTest, StaticCertificateValidationContext) {
  ...
  const std::string certificate_validation_context_yaml = R"EOF(
    name: "def.com"
    validation_context:
      trusted_ca: { filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ca_cert.pem" }
  ...
  const std::string cert_pem = "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ca_cert.pem";
  EXPECT_EQ(TestEnvironment::readFileToStringForTest(TestEnvironment::substitute(cert_pem)),
...
TEST(ServerContextConfigImplTest, MultipleTlsCertificates) {
  ...
  NiceMock<Server::Configuration::MockTransportSocketFactoryContext> factory_context;
  EXPECT_THROW_WITH_MESSAGE(
	  Envoy::Ssl::ServerContextConfigImpl client_context_config(tls_context, factory_context), EnvoyException,
...
TEST(ServerContextConfigImplTest, TlsCertificatesAndSdsConfig) {
  ...
  NiceMock<Server::Configuration::MockTransportSocketFactoryContext> factory_context;
  EXPECT_THROW_WITH_MESSAGE(
	  Envoy::Ssl::ServerContextConfigImpl server_context_config(tls_context, factory_context), EnvoyException,
      "No TLS certificates found for server context");
  const std::string tls_certificate_yaml = R"EOF(
  certificate_chain:
    filename: "{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/selfsigned_cert.pem"
...
TEST(ServerContextConfigImplTest, SecretNotReady) {
  ...
  sds_secret_configs->set_name("abc.com");
  sds_secret_configs->mutable_sds_config();
  Envoy::Ssl::ServerContextConfigImpl server_context_config(tls_context, factory_context);
  // When sds secret is not downloaded, config is not ready.
...
TEST(ServerContextImplTest, TlsCertificateNonEmpty) {
  ...
  NiceMock<Server::Configuration::MockTransportSocketFactoryContext> factory_context;
  tls_context.mutable_common_tls_context()->add_tls_certificates();
  Envoy::Ssl::ServerContextConfigImpl client_context_config(tls_context, factory_context);
  Event::SimulatedTimeSystem time_system;
  Envoy::Ssl::ContextManagerImpl manager(time_system);
  Stats::IsolatedStoreImpl store;
  EXPECT_THROW_WITH_MESSAGE(Envoy::Ssl::ServerContextSharedPtr server_ctx(manager.createSslServerContext(
...
TEST(ServerContextConfigImplTest, InvalidIgnoreCertsNoCA) {
  ...
  EXPECT_NO_THROW(Envoy::Ssl::ServerContextConfigImpl server_context_config(tls_context, factory_context));
  ...
  server_validation_ctx->mutable_trusted_ca()->set_filename(
      TestEnvironment::substitute("{{ test_rundir }}/test/extensions/transport_sockets/openssl/test_data/ca_cert.pem"));
