...
#include "extensions/transport_sockets/openssl/utility.h"
#include "extensions/transport_sockets/openssl/openssl_impl.h"
...
ContextImpl::ContextImpl(Stats::Scope& scope, const Envoy::Ssl::ContextConfig& config, TimeSource& time_source)
  ...
  for (auto& ctx : tls_contexts_) {
    ...
    if (!Envoy::Extensions::TransportSockets::Openssl::set_strict_cipher_list(ctx.ssl_ctx_.get(), config.cipherSuites().c_str())) {
      ...
      for (const auto& cipher : ciphers) {
        ...
        if (cipher_str.compare("-ALL") && cipher_str.compare("ALL")){
          if (!Envoy::Extensions::TransportSockets::Openssl::set_strict_cipher_list(ctx.ssl_ctx_.get(), cipher_str.c_str())) {
  ...
  if (config.certificateValidationContext() != nullptr &&
    ...
    for (auto& ctx : tls_contexts_) {
      ...
      for (const X509_INFO* item : list.get()) {
        ...
        Envoy::Extensions::TransportSockets::Openssl::ssl_ctx_add_client_CA(ctx.ssl_ctx_.get(), item->x509);
...
bssl::UniquePtr<SSL> ContextImpl::newSsl(absl::optional<std::string>) {
  ...
  //return Envoy::Ssl::newSsl(ctx_.get());
...
int ContextImpl::verifyCallback(X509_STORE_CTX* store_ctx, void* arg) {
  ...
  X509* cert = Envoy::Extensions::TransportSockets::Openssl::getVerifyCallbackCert(store_ctx, arg);
...
Envoy::Ssl::CertificateDetailsPtr ContextImpl::certificateDetails(X509* cert, const std::string& path) const {
...
ClientContextImpl::ClientContextImpl(Stats::Scope& scope, const Envoy::Ssl::ClientContextConfig& config,
...
bssl::UniquePtr<SSL> ClientContextImpl::newSsl(absl::optional<std::string> override_server_name) {
  ...
  if (allow_renegotiation_) {
	  Envoy::Extensions::TransportSockets::Openssl::allowRenegotiation(ssl_con.get());
  }
  ...
  if (max_session_keys_ > 0) {
    if (session_keys_single_use_) {
      ...
      if (!session_keys_.empty()) {
        ...
        if (Envoy::Extensions::TransportSockets::Openssl::should_be_single_use(session)) {
...
ServerContextImpl::ServerContextImpl(Stats::Scope& scope, const Envoy::Ssl::ServerContextConfig& config,
  ...
  // BoringSSL.
  Envoy::Extensions::TransportSockets::Openssl::set_select_certificate_cb(tls_contexts_[0].ssl_ctx_.get());
...
//      SSL_CTX_set_client_CA_list(ctx_.get(), list.release());
    ...
    {
      ContextImpl* context_impl = static_cast<ContextImpl*>(SSL_CTX_get_app_data(SSL_get_SSL_CTX(ssl)));
...
void ServerContextImpl::generateHashForSessionContexId(const std::vector<std::string>& server_names,
  ...
  EVP_MD_CTX *md = Envoy::Extensions::TransportSockets::Openssl::newEVP_MD_CTX();
...
int ServerContextImpl::sessionTicketProcess(SSL*, uint8_t* key_name, uint8_t* iv,
  ...
  if (encrypt == 1) {
    ...
    RELEASE_ASSERT((int)key.aes_key_.size() == EVP_CIPHER_key_length(cipher), "");
...
//      [](const X509_NAME** a, const X509_NAME** b) -> int { return X509_NAME_cmp(*a, *b); }));
  bssl::UniquePtr<STACK_OF(X509_NAME)> list = Envoy::Extensions::TransportSockets::Openssl::initX509Names();
